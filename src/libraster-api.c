...
void _draw_text_box(Box *box, draw_rectangle_callback_t draw_rectangle,
...
void render_interface(Box *text_boxes, uint16_t num,
#if RASTER_BATCH_GLYPHS
                      draw_batch_at_position_t draw_batch_at_position,
                      uint8_t *alphas,
#else
...
#endif
    for (int i = 0; i < num; i++) {
        _draw_text_box(text_boxes + i, draw_rectangle,
#if RASTER_BATCH_GLYPHS
                       alphas,
                       draw_batch_at_position
#else
