...
enum RasterReturnCode box_api_draw(const struct Box *box, raster_draw_rectangle_callback draw) {
    ...
    if (box->label != NULL) {
        const uint16_t label_x = (uint16_t)((int32_t)box->rect.x + box->label->offset_x);
        const uint16_t label_y = (uint16_t)((int32_t)box->rect.y + box->label->offset_y);
        if (label_api_draw(box->label, label_x, label_y, draw) != RASTER_RC_OK) {
            return RASTER_RC_ERROR;
        }
    }
    return RASTER_RC_OK;
